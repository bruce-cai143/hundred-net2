...
router.get('/files', async (req, res) => {
    try {
        const { category, type, limit = 20, offset = 0, sort = 'upload_date', order = 'DESC' } = req.query;
        ...
        let query = `SELECT id, title, description, file_name, file_size, file_type, upload_date, category FROM ${tableName} WHERE status = 1`;
        let countQuery = `SELECT COUNT(*) as total FROM ${tableName} WHERE status = 1`;
